...
if __name__ == "__main__":
    ...
    ratio_h.GetXaxis().SetTitle("Lxy [cm]")
    ratio_h.SetLineColor(2)
    ratio_h.GetYaxis().SetTitle("eff.")
    ratio_h.SetTitle("hps-tau/gen-tau")
    ratio_h.Draw("histo")
    ratio_h_jet = DrawUtils.createRatio(h1_Tau_h_jet, h1_Tau_h_all,"eff.")
    ratio_h_jet.GetXaxis().SetTitle("Lxy [cm]")
    ratio_h_jet.GetYaxis().SetTitle("eff.")
    ratio_h_jet.SetLineColor(4)
    ratio_h_jet.SetTitle("reco-jet/gen-tau")
    ratio_h_jet.Draw("histosame")
    DrawUtils.DrawHeader(pad1, "dR<0.4; pt_rel<20%" , "", "c#tau_{0}=100mm")
    legend2 = DrawUtils.GetHistTitlesLegend([ratio_h, ratio_h_jet])
    legend2.Draw()
    canvas.SaveAs("efficiency_main_all.png")
